  ...
  while (1) {
    ...
    for (i = 0; i < sizeof(keys); i++) {
      if (fromCompy == keys[i]) {       /* found match in lookup table */
        playNote(notes[i]+8, currentNoteLength/3);
		playNote(notes[i]-32, currentNoteLength/2);
		playNote(notes[i]+128, currentNoteLength/3);
		playNote(notes[i]-78, currentNoteLength/2);
